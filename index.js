...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        newDiv.innerHTML = `
            ...
            <h3>${games[i].name}</h2>
            <p><i>${games[i].description}</i></p>
            <p><b>BACKERS: </b>${games[i].backers}</p>
            <p><b>PLEDGED: </b>${games[i].pledged}</p>
            <p><b>GOAL: </b>${games[i].goal}</p>
        `;
        // about each game
...
let unfundedGamesCnt = GAMES_JSON.reduce((acc, game) => {
    return ((game.pledged < game.goal) ? (acc + 1) : acc); 
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${totalGames} games. At this moment, ${unfundedGamesCnt} ${unfundedGamesCnt > 1 ? 'games remain':'game remains'} unfunded. We need your help to fund these awesome games!`;
...
descriptionContainer.append(newDiv);
